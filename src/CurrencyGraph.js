...
const CurrencyGraph = ({ country }) => {
  ...
  React.useEffect(() => {
    ...
    const dataFilteredByYear = Object.entries(countryData)
      .filter(([key]) => key !== "Country_Name" && key !== "Country_Code" && key !== "2021")
      .map(([key, value]) => ({
        year: key,
        value: value
      }));
  ...
  return (
    ...
    <React.Fragment>
      <Typography variant="h6" align="center">{country} Currency Values Over Time</Typography>
      <ResponsiveContainer width="100%" height={250}>
        <LineChart data={filteredData} margin={{ top: 20, right: 50, left: 10, bottom: 10 }}>
          <XAxis dataKey="year" />
          <YAxis />
          <Tooltip />
          <Legend />
          <Line type="basis" dataKey="value" stroke="#8884d8" activeDot={{ r: 7 }} />
        </LineChart>
        </ResponsiveContainer>
    </React.Fragment>
    </Card>
